...
The plugin provides a way to track events and their status (read or unread). The events are not the same as the ones triggered by the
`qa_report_event()` function. The events are just records in a table. This way, the notifications can be generated without having to
create a record in the `^eventlog` table.
 ...
 * `plugin_id`: Textual ID set by a plugin identifying their plugin. This plugin would be `pupi_fns`. As this plugin doesn't generate
...
There are two relevant use cases: plugin developers that want to trigger notifications from their plugins and theme developers trying
to style the notifications system properly to their theme. Both cases will be covered next.
...
I will show a real life example of how to implement this with a plugin that triggers a notification whenevere a user is mentioned in
a post using an `@`.
...
```json
{
    ...
    "url": "http:\/\/lsite.com\/10059\/another-question?show=10093#c10093",
...
Furthermore, some information could be stored and fetched from the database, while other could be generated by PHP code. Whether you
decided to go one way or the other, it is up to you. Focus on efficiency and avoid unnecessary database accesses.

For example, I don't think it is a good idea to store URLs. They should be dynamically generated. Otherwise, if the site URL changes,
the notification will link to an incorrect page. So as I want to link to a post, all I need is the question the post belongs to (note
the title is not a must but might save a redirect), the post type and the mentioned user handle, so I can display a text that looks
like "user1 mentioned in a post".

Other fields can be generated in PHP. There is no need to store the `event_name`, `name` or the `icon` in a database. In fact, future
releases of your plugin might need to change that would need changing the content of the already generated databases.
...
The notification is inserting using the `addNotification()` method of the `PUPI_FNS_NotificationsService` class. The parameters of the
 ...
 * `array $params`: 1-dimension array containing the parameters that will be stored in a JSON format (the ones we have to store in the
   database)
...
In my case, I insert notifications in this way:
...
Note that the ID comes from another class I will explain later.

For simplicity, the parameters need to be a 1-dimension array. You control the content in any other way aside from that. If the
parameters exceed the database field size, I've added a logic that will cut the longest strings in the array until it fits the size.
So is better if you keep it short.
...
structure the params and turn them into something closer to the original notification structure, we need to process the parameters.
...
but before sending the endpoint response to the user. This allows plugins to hook in the middle of the generation of the notifications.

In order to register an extension to the FNS, the plugin will have to do register the plugin creating a `process` module, like this:
...
In my case, the extension will look like this:
...
notification as it is, in case it does not belong to them. Then, structure of the final notification is created. The `switch`
statement is unnecessary as there is only one notification, but I left the door open for more to come.
...
Note the `icon` also references a class. In my case, again, I used Fontello to create a font just for that icon. There are other
alternative approaches, however, you can't rely on the theme developer to even know you are following one of them.
